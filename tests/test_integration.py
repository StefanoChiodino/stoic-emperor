...
class TestIngestionPipelineIntegration:
    ...
    def test_query_ingested_highlights(self, integration_test_config):
        ...
        results = vectors.query(collection="stoic_wisdom", query_texts=["control over my thoughts"], n_results=3)
...
class TestFullChatWorkflow:
    def test_chat_session_workflow(self, integration_test_config):
        ...
        _vectors = VectorStore(integration_test_config["vector_path"])
        llm = LLMClient(base_url=integration_test_config.get("base_url"))
        ...
        user_msg = Message(session_id=session.id, role="user", content="I feel like I'm not good enough for my job.")
        db.save_message(user_msg)

        response = brain.respond(user_message=user_msg.content, conversation_history=[user_msg], retrieved_context=None)
    ...
    def test_multi_turn_conversation(self, integration_test_config):
        ...
        for user_text in conversation:
            ...
            response = brain.respond(user_message=user_text, conversation_history=history, retrieved_context=None)
