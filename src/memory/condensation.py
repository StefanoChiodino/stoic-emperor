...
class CondensationManager:
    ...
    def get_uncondensed_messages(self, user_id: str) -> list[Message]:
        ...
        uncondensed = self.db.get_messages_in_range(user_id, start_date=start_date, end_date=cutoff_date)
    ...
    def condense_chunk(self, user_id: str, messages: list[Message]) -> CondensedSummary | None:
        if not messages:
            return None
        ...
        messages_text = "\n\n".join(
            [f"[{msg.created_at.strftime('%Y-%m-%d %H:%M')}] {msg.role.upper()}: {msg.content}" for msg in messages]
        )
    ...
    def condense_summaries(self, user_id: str, level: int) -> CondensedSummary | None:
        summaries = self.db.get_condensed_summaries(user_id, level=level)
        if len(summaries) <= 1:
            return None
        ...
        for level in range(max_level, 0, -1):
            ...
            for summary in level_summaries:
                covered_by_higher = any(
                    s.period_start <= summary.period_start and s.period_end >= summary.period_end for s in selected
                )
