...
class UnifiedRetriever:
    def __init__(self, db: Database, vectors: VectorStore, brain: EmperorBrain, max_context_tokens: int = 4000):
        self.db = db
    ...
    def retrieve(self, user_id: str, session_id: str, user_message: str, n_results: int = 5) -> RetrievalContext:
        expanded_query = self._expand_query(user_message)

        recent_messages = self.episodic.get_recent_context(session_id=session_id, current_message=user_message)

        episodic_matches = self.episodic.search_past_conversations(
            user_id=user_id, query=expanded_query, n_results=n_results
        )
    ...
    def _query_collection(self, collection: str, query: str, n_results: int) -> list[str]:
        try:
            results = self.vectors.query(collection=collection, query_texts=[query], n_results=n_results)
